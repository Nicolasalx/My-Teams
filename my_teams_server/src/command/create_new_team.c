...
static void send_new_team_created(int fd,
    server_t *server, db_team_t *new_team)
{
    ...
    memcpy(reply_data.arg3.team_description,
        new_team->description, MAX_DESCRIPTION_LENGTH);
...
void create_new_team(server_t *server,
    client_t *client, cmd_data_t *cmd_data)
{
    ...
    if (db_contain_team_name(&server->database, cmd_data->arg1.team_name)) {
        ...
        memcpy(new_team->description,
            cmd_data->arg2.team_description, MAX_DESCRIPTION_LENGTH);
        ...
        server_event_team_created(
            new_team->uuid, new_team->name, client->uuid);
