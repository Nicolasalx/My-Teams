...
static void send_send_reply(int fd,
    const char *user_uuid, const char *message_body)
{
...
static void fill_private_msg(db_private_msg_t *private_msg,
...
void cmd_send(server_t *server, client_t *client, cmd_data_t *cmd_data)
{
    ...
    db_user_t *dest_user =
        db_contain_user_uuid(&server->database, cmd_data->arg1.user_uuid);
    client_t *dest_client = NULL;

    if (dest_user) {
        fill_private_msg(private_msg, client, cmd_data);
        append_node(&server->database.private_msg_list,
            create_node(private_msg));
        server_event_private_message_sended(client->uuid,
            cmd_data->arg1.user_uuid, cmd_data->arg2.message_body);
        dest_client = get_client_by_uuid(server, private_msg->receiver_uuid);
        if (dest_client) {
            send_send_reply(dest_client->fd,
            private_msg->receiver_uuid, private_msg->msg);
        }
    } else {
