...
static void send_new_thread_created(client_t *client,
    server_t *server, db_team_t *team, db_thread_t *new_thread)
{
    ...
    memcpy(&reply_data.arg3.thread_timestamp,
        &new_thread->timestamp, sizeof(time_t));
    ...
    client_t *client, cmd_data_t *cmd_data)
{
    uuid_t thread_uuid = {0};
        ...
        append_node(&GET_DATA(client->context.channel_ptr,
            db_channel_t)->thread_list, create_node(new_thread));
}

void create_new_thread(server_t *server,
    client_t *client, cmd_data_t *cmd_data)
{
    ...
    if (db_contain_thread_name(GET_DATA(client->context.channel_ptr,
        db_channel_t), cmd_data->arg1.thread_title)) {
        send_error_already_exist(client->fd);
    } else {
        new_thread = my_calloc(sizeof(db_thread_t));
        fill_new_thread(new_thread, client, cmd_data);
        server_event_thread_created(client->context.channel_uuid,
        new_thread->uuid, client->uuid, new_thread->name, new_thread->body);
        send_new_thread_created(client, server,
            GET_DATA(client->context.team_ptr, db_team_t), new_thread);
    }
}
